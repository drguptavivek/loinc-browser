...
- Term search, panel search, term detail, and `/api/v1/terms/match` accept `lang=de-DE` (etc.) and return `localizedName`, which is display only; searching in the language is not supported yet. `/api/version` lists `languages` and the loaded `commonCodes` label and count.
