...
- Added **Ask AI**: agentic search in Find. Describe a test in plain words and a model on any OpenAI-compatible endpoint (LM Studio, Ollama, vLLM, a hosted gateway) searches the local LOINC data through the MCP tools and answers with ranked codes and reasons. **My setup → AI assistant** lists the endpoint's models to pick from (tested with `google/gemma-4-e2b` and `medgemma-27b-text-it-mlx` on LM Studio), stores an optional API key encrypted, runs **Test connection** (including tool support), and sets **Thinking** on or off, which can also be changed per conversation. Only codes that tool results returned and the database confirms are shown as cards; any other code in the answer is flagged "not verified". Config: `LOINC_AGENT_LLM_BASE_URL`, `_MODEL`, `_API_KEY`, `_THINKING`, `_LOCAL_ONLY` (on by default: loopback and private addresses only, checked when each connection is made, with no redirects followed), and `LOINC_AGENT_DISABLED`. The API key is sent only to the saved endpoint, cross-origin requests to agent and official routes are refused, and the `/api/v1/agent/*` routes are in `docs/API.md`. Scope is phase A2 of `docs/AGENT_CHAT_PLAN.md`; the decision store and CSV agent tools are deferred, since Map a list covers batch mapping.
- Added the MCP tool `loinc_match_names` (batch matching for agents).
- Side drawers (term card, basket, setup, Ask AI) now sit above the desktop footer.
