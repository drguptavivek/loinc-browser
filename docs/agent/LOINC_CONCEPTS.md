...
- Mapping guidance: see `LOINC_MAPPING_GUIDANCE.md` for `mapping_guidance`, `mapping_workflow`, `units_and_property`, `calculated_vs_measured`, `detection_limit_variants`, `pregnancy_and_hcg`, `timed_urine`, `blood_gas_specimens`, `coagulation_specimens`, `heparin_and_lupus_anticoagulant`, `cbc_and_differential`, `drug_screen_vs_confirm`, `viral_load_and_sti_testing`, `occult_blood`, `allergen_synonyms`, and `cell_marker_names` (lab-mapping rules from LOINC's Top 2000 mapper's guide).
- India: see `LOINC_CLCI.md` for `clci`, `clci_in_this_app`, `blood_means_serum_or_plasma`, `clci_evaluation`, and `updating_clci` (Common Lab Codes for India, the NRCeS national subset: ranking prior, `clci` filter, `clciName`).
- Official API: see `LOINC_OFFICIAL_API.md` for `official_api_search`, `official_api_credentials`, `official_api_query_syntax`, `official_api_use_guidance`, and the local Search API–compatible endpoint (`GET /searchapi/{scope}`).
- FHIR terminology surface: this app also serves a local, wire-compatible clone of `https://fhir.loinc.org` at `/fhir/...` — CodeSystem `$lookup`/`$validate-code`/`$subsumes`, ValueSets (including LL answer lists, LG groups, and the HL7 implicit multiaxial-hierarchy `vs/{LP}` sets), ConceptMaps, and Questionnaires for LOINC panels. See `../FHIR_TERMINOLOGY_PLAN.md` for the full wire contract and `../LOCAL_APIS.md` for a consumer quick reference; this KB does not duplicate that material. Sources: [LOINC FHIR Terminology Server](https://loinc.org/fhir/) and [HL7 "Using LOINC with FHIR"](https://terminology.hl7.org/LOINC.html).
