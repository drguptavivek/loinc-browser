...
Never delete, move, or overwrite anything in `data/` (release data, databases, `loinc-browser-kv.json` saved settings and encrypted credentials, `loinc-browser-app.key`); none of it is in git, so a deletion cannot be undone. For manual test runs, point the server at scratch paths, for example `LOINC_KV_PATH=/tmp/…/kv.json`, instead of cleaning up files in `data/` afterwards.
